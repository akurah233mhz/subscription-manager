...
| `paymentMethod` | Select | クレジットカード / Apple / Google / PayPal / 銀行口座 / 携帯決済 / その他 |
| `plan`         | Text     |                                                    |
...
契約者の追加・変更は、Notion側の `contractOwner` Select options を更新する。アプリはWorkersの `/api/meta` 経由でSelect optionsを取得するため、通常はコード変更不要。

支払い方法の追加・変更は、Notion側の `paymentMethod` Select options を更新する。アプリはWorkersの `/api/meta` 経由でSelect optionsを取得するため、通常はコード変更不要。
